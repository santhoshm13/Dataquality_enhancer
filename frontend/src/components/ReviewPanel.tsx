...
import { AlertTriangle, RefreshCw, Search, Globe, Cpu, Sparkles, Filter, ShieldAlert, CheckCircle2, Play, Edit3, Check, X } from 'lucide-react';
                      ...
                      <td className="p-4 pr-5 text-right">
                        <div className="flex flex-col items-end gap-2">
                          {correctionSaved.has(p.id) ? (
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 text-xs font-bold">
                            ...
                            <>
                              <div className="flex items-center gap-2">
                                <motion.button
                                  ...
                                  className="btn-primary inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-white font-bold text-xs cursor-pointer shadow-lg"
                                >
                                ...
                                <button
                                  ...
                                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-500/15 text-amber-300 border border-amber-500/30 text-xs font-bold hover:bg-amber-500/25 cursor-pointer transition-colors"
                                >
                              ...
                              {correcting === p.id && (
                                <div className="bg-black/80 border border-amber-500/30 rounded-xl p-3 w-64 space-y-2 text-xs">
                                  <p className="text-amber-300 font-bold text-[11px] uppercase tracking-wider">Inline Correction</p>
                                  <select
                                    value={correctionField}
                                    onChange={e => setCorrectionField(e.target.value)}
                                    className="w-full bg-black/60 border border-white/15 rounded-lg px-2.5 py-1.5 text-slate-200 text-xs font-mono cursor-pointer"
                                  >
                                  ...
                                  <input
                                    ...
                                    placeholder="Canonical corrected value..."
                                    className="w-full bg-black/60 border border-white/15 rounded-lg px-2.5 py-1.5 text-white text-xs font-mono placeholder:text-slate-600"
                                    onKeyDown={e => { if (e.key === 'Enter') submitCorrection(p.id, p.manufacturer || ''); }}
                                  />
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => submitCorrection(p.id, p.manufacturer || '')}
                                      className="flex-1 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs cursor-pointer transition-colors"
                                    >
                                      Save Override
                                    </button>
                                    <button
                                      onClick={() => setCorrecting(null)}
                                      className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-slate-400 cursor-pointer"
                                    >
